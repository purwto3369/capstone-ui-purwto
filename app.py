...
def index():
    ...
    top_category = pd.crosstab(
        index=df2['Category'],
        columns='Jumlah').sort_values(by = "Jumlah", ascending=False).reset_index().head()
    # Dictionary stats digunakan untuk menyimpan beberapa data yang digunakan untuk menampilkan nilai di value box dan tabel
    stats = {
        'most_categories' : top_category['Category'].loc[0],
        'total': top_category.loc[0,'Jumlah'],
        'rev_table' : df2.groupby(['Category','App']).agg({'Reviews':'sum','Rating':'mean'}).sort_values('Reviews', ascending=False).\
            reset_index().head(10).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
